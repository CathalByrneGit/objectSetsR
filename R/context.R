...
symbols_in_select_expr <- function(expr) {
  if (rlang::is_call(expr)) {
    name <- rlang::call_name(expr)
    if (name %in% c("=", ":=")) {
      return(symbols_in_select_expr(expr[[3]]))
    }
  }
...
validate_select_exprs <- function(object_type, quos) {
  symbols <- unique(unlist(lapply(quos, function(q) symbols_in_select_expr(rlang::get_expr(q)))))
  validate_property_ids(object_type, symbols)
}
